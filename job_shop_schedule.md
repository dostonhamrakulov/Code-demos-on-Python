...
## :black_nib: Objective <br>

Here is to explain how to use GA to solve the problem of Job shop, the following will be a brief introduction to the Job shop problem, and then describe the solution problem of this example and coding and decoding instructions, and finally according to each program block for conceptual explanation

### :arrow_down_small: What is the Job shop problem? <br>

The biggest difference between the Jop shop problem and the Flow shop problem is that, unlike the Flow shop problem, each workpiece is processed in the same order in the machine, and in the Job shop problem, each artifact has its own machine machining order, as shown in the following figure:<br>
...
This example is a 10x10 Job shop problem, a total of 10 workpieces and 10 sets of machines, each workpiece in each machine processing order is different, the scheduling target is to minimize the total completion time (Makespan), the data is in the workpiece processing sequence to present, each workpiece will go through 10 machining operations , The following table records the machining machines of each workpiece in each machining operation and the time required for processing<br>
...
The goal of this example is to minimize the total completion time (Makespan), that is, to minimize the uptime of the entire schedule, taking the previous example as an example, the completion time of the example is Job 3 at the completion point of the Machine 1, because for this scheduling result, Job 3 is the last of all artifacts completed, So the completion time of this schedule is the point in time when Job 3 is completed.
...
Refences [Gen-Tsujimura-Kubota’s Method (1994, 1997)](https://ieeexplore.ieee.org/document/400072/)所提出的 Job shop 排程問題的染色體編碼方式。<br>

The concept of this method is to represent the chromosome as a set of workpiece processing sequence, a gene represents the processing operation of a workpiece, according to the number of chromosomes in the appearance of the workpiece, to know the current processing of each workpiece, and then to correspond to the workpiece processing machine and processing time, in order to carry out the scheduling.<br>
